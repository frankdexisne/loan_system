        ...
        <div class="clearfix">
            <div class="pull-left tableTools-container">
                <div class="form-inline">
                    ...
                    <div class="form-group">
                        <label for="">To release at</label>
                        <input type="date" class="form-control form-inline" id="to_release_at" name="to_release_at">
                    </div>
                </div>
            </div>
            <div class="pull-right tableTools-container">
                <div class="dt-buttons btn-overlap btn-group">
                    <a
                        id="print"
                        class="dt-button btn btn-white btn-default btn-bold"
                        title="Print For Collection"
                        >
                        <span>
                            <i class="fa fa-print bigger-110 black"></i>
                        </span>
...
@include($module.'.for_releasing_modal',['title' => 'Releasing', 'name' => 'date_release', 'label' => 'Release date'])
@include($module.'.modal_loan_report')
...
<script>
    $(document).ready(function(){
        ...
        function initParentTable(){
            ...
            dTableParams.ajax = {
                ...
                data: {
                    ...
                    to_release_at: $('#to_release_at').val()
                }
            };
            dTableParams.columns = [

                {data : 'transaction_code', title : '#'},
                {data: 'included_at', title: 'Include', render(data){
                    var checked = data !== '0000-00-00 00:00:00' ? 'checked' : '';
                    return '<input type="checkbox" class="include" '+checked+'>';
                }},
                ...
                {data : null, title : 'Loan Amount', render(data) {
                    return data['loan_amount_formatted'] + '<br>' +
                        'For release at :' + data['to_release_at'];
                }},

                {data : null, title : 'Action', render(data,type){
                    var showRelease = data['included_at'] === '0000-00-00 00:00:00' ? 'hidden' : '';
                    return '<div class="action-buttons">'+
                                '<a class="default voucher" href="#" title="Voucher">'+
                                    '<i class="ace-icon fa fa-file bigger-130"></i>'+
                                '</a>'+
                                '<a class="blue release '+showRelease+'" href="#">'+
        ...
        $('#payment_mode_id').change(function(){
            ...
            dTableParams.ajax.data.to_release_at= $('#to_release_at').val();
        ...
        function tableOnChange()
        {
            ...
            datatable.on('change', 'input[type="checkbox"]', function(e) {
                e.preventDefault();
                $tr = $(this).closest('tr');
                var type = $(this).is(':checked') ? 'included' : 'excluded';
                var data = datatable.row($tr).data();
                data['included_at'] = $(this).is(':checked') ? '2021-10-09 15:07:00' : null
                $.ajax({
                    url: "{{url($module.'/commit-include')}}/"+data['id']+'/'+type,
                    type: 'PATCH',
                    success: function(){
                        datatable.row($tr).data(data).invalidate();
                    }
                });
            });
